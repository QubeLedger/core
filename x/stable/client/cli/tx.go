...
func NewRegisterDeletePairProposalCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "delete-pair [pairid]",
		Args:  cobra.ExactArgs(1),
		Short: "Submit a delete pair proposal",
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
				return err
			...
			content := types.NewRegisterDeletePairProposal(title, description, args[0])
