...
func (rtbp *RegisterCoinProposal) ValidateBasic() error {
	...
	err1 := validateIBCVoucherMetadata(rtbp.Metadata)
	err2 := validateFactoryVoucherMetadata(rtbp.Metadata)

	if err1 != nil && err2 != nil {
		return fmt.Errorf("invalid metadata. denomination should be prefixed with the format 'ibc/' or 'factory/'")
	}
...
func validateIBCVoucherMetadata(metadata banktypes.Metadata) error {
	...
	if len(denomSplit) != 2 || denomSplit[0] != ibctransfertypes.DenomPrefix {
		...
		return fmt.Errorf("invalid metadata. %s denomination should be prefixed with the format 'ibc/', got %s expected 'ibc/'", metadata.Base, denomSplit[0])
	}

	return nil
}

func validateFactoryVoucherMetadata(metadata banktypes.Metadata) error {
	// Check factory/ denom
	denomSplit := strings.SplitN(metadata.Base, "/", 2)
	...
	if len(denomSplit) != 2 || denomSplit[0] != tokenfactorytypes.ModuleDenomPrefix {
		// NOTE: should be unaccessible (covered on ValidateFactoryDenom)
		return fmt.Errorf("invalid metadata. %s denomination should be prefixed with the format 'factory/'. got %s expected 'factory/'", metadata.Base, denomSplit[0])
	}
