...
func (suite *Erc20GenesisTestSuite) SetupTest() {
	// consensus key
	pk := ed25519.GenPrivKey().PubKey()
	consAddress := sdk.ConsAddress(sdk.AccAddress(pk.Address()))

	suite.app = quadrateapptest.Setup(suite.T(), "quadrate_5120-1", false, 1)
...
func (suite *Erc20GenesisTestSuite) TestERC20InitGenesis() {
	testCases := []struct {
		name         string
...
func (suite *Erc20GenesisTestSuite) TestErc20ExportGenesis() {
	testGenCases := []struct {
		name         string
		genesisState types.GenesisState
	}{
...
func TestGenesisTestSuite(t *testing.T) {
	suite.Run(t, new(apptesting.Erc20GenesisTestSuite))
}
