...
func (k Keeper) GenerateTraderPositionId(creator string, denom string, trading_asset_id string, trade_type types.PerpetualTradeType) string {
	return fmt.Sprintf("%x", crypto.Sha256(append([]byte(creator+denom+trading_asset_id+trade_type.String()))))
}
