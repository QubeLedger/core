...
The `Operator` field contains the operator address of the validator (prefixed `qubevaloper-`). The `Delegate` field is the account address (prefixed `terra-`) of the delegate account that will be submitting exchange rate related votes and prevotes on behalf of the `Operator`.
