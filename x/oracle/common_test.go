...
func setupWithSmallVotingPower(t *testing.T) (keeper.TestInput, types.MsgServer) {
	...
	_, err := sh.CreateValidator(sdk.WrapSDKContext(input.Ctx),
...
func setup(t *testing.T) (keeper.TestInput, types.MsgServer) {
	...
	// Validator created
	_, err := sh.CreateValidator(sdk.WrapSDKContext(input.Ctx), keeper.NewTestMsgCreateValidator(keeper.ValAddrs[0], keeper.ValPubKeys[0], stakingAmt))
	require.NoError(t, err)
	_, err = sh.CreateValidator(sdk.WrapSDKContext(input.Ctx), keeper.NewTestMsgCreateValidator(keeper.ValAddrs[1], keeper.ValPubKeys[1], stakingAmt))
	require.NoError(t, err)
	_, err = sh.CreateValidator(sdk.WrapSDKContext(input.Ctx), keeper.NewTestMsgCreateValidator(keeper.ValAddrs[2], keeper.ValPubKeys[2], stakingAmt))
	require.NoError(t, err)
	staking.EndBlocker(input.Ctx, input.StakingKeeper)
...
func setupVal5(t *testing.T) (keeper.TestInput, types.MsgServer) {
	...
	_, err = sh.CreateValidator(sdk.WrapSDKContext(input.Ctx), keeper.NewTestMsgCreateValidator(keeper.ValAddrs[2], keeper.ValPubKeys[2], stakingAmt))
	require.NoError(t, err)
	_, err = sh.CreateValidator(sdk.WrapSDKContext(input.Ctx), keeper.NewTestMsgCreateValidator(keeper.ValAddrs[3], keeper.ValPubKeys[3], stakingAmt))
	require.NoError(t, err)
	_, err = sh.CreateValidator(sdk.WrapSDKContext(input.Ctx), keeper.NewTestMsgCreateValidator(keeper.ValAddrs[4], keeper.ValPubKeys[4], stakingAmt))
	require.NoError(t, err)
	staking.EndBlocker(input.Ctx, input.StakingKeeper)
