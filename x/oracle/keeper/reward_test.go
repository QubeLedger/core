...
func TestRewardBallotWinners(t *testing.T) {
	...
	// Validator created
	_, err := sh.CreateValidator(sdk.WrapSDKContext(input.Ctx), NewTestMsgCreateValidator(addr, val, amt))
	require.NoError(t, err)
	_, err = sh.CreateValidator(sdk.WrapSDKContext(input.Ctx), NewTestMsgCreateValidator(addr1, val1, amt))
	require.NoError(t, err)
