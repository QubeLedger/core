...
func setup(t *testing.T) (TestInput, types.MsgServer) {
	...
	// Validator created
	_, err := sh.CreateValidator(sdk.WrapSDKContext(input.Ctx), NewTestMsgCreateValidator(ValAddrs[0], ValPubKeys[0], stakingAmt))
	require.NoError(t, err)
	_, err = sh.CreateValidator(sdk.WrapSDKContext(input.Ctx), NewTestMsgCreateValidator(ValAddrs[1], ValPubKeys[1], stakingAmt))
	require.NoError(t, err)
	_, err = sh.CreateValidator(sdk.WrapSDKContext(input.Ctx), NewTestMsgCreateValidator(ValAddrs[2], ValPubKeys[2], stakingAmt))
	require.NoError(t, err)
