...
import (
	...
	"github.com/QuadrateOrg/core/utils/addressutils"
...
func GetSimApp(chain *ibctesting.TestChain) *app.QuadrateApp {
	quicksilver, ok := chain.App.(*app.QuadrateApp)
	if !ok {
...
func TestMsgSubmitQueryResponse(t *testing.T) {
	...
	msg := types.MsgSubmitQueryResponse{
		...
		QueryId:     keeper.GenerateQueryHash(path.EndpointB.ConnectionID, chainB.ChainID, "cosmos.staking.v1beta1.Query/Validators", bz, "", sdk.AccAddress{}),
