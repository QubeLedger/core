...
func TestHostChain_GetValidator(t *testing.T) {
	foundvaloper := NewModuleAddressOrBech32Address("testval1").String()
	...
	tests := []struct {
		...
		},
		{
			...
			hostChain: func() types.HostChain {
				hc := validHostChain()
				hc.Validators = append(hc.Validators, makeVal(NewModuleAddressOrBech32Address("testval4").String()))
				return *hc
			},
			args:  args{operatorAddress: foundvaloper},
			want:  nil,
...
func validHostChain() *types.HostChain {
	return &types.HostChain{
		...
		Validators: []*types.Validator{{
			OperatorAddress: NewModuleAddressOrBech32Address("testval2").String(),
			...
			OperatorAddress: NewModuleAddressOrBech32Address("testval3").String(),
			Status:          stakingtypes.BondStatusBonded,
