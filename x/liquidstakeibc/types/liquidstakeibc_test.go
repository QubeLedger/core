...
func TestHostChain_Validate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			fields: func() fields {
				newfields := validFields()
				newfields.HostDenom = "me coin much"
				return newfields
			},
			wantErr: true,
		},
...
func TestUserUnbonding_Validate(t *testing.T) {
	...
	validCoin := sdk.NewInt64Coin("qs/uatom", 1000)
	invalidCoin := validCoin
	invalidCoin.Amount = sdk.NewInt(-1000)
	tests := []struct {
		name    string
...
func TestMintDenomToHostDenom(t *testing.T) {
	hostDenom, found := types.MintDenomToHostDenom("qs/uatom")
...
func TestRewardParams_Validate(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				Denom:       "uatom",
				Destination: NewModuleAddressOrBech32Address("addr").String(),
			},
			wantErr: false,
		},
		{
