...
func NewLiquidUnstakeCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: strings.TrimSpace(
			fmt.Sprintf(
				`Submit a liquid unstake transaction: $ %s tx liquidstakeibc liquid-unstake 100000000qs/uatom`,
				version.AppName,
			),
		),
		Args: cobra.ExactArgs(1),
...
func NewRedeemCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: strings.TrimSpace(
			fmt.Sprintf(
				`Submit a redeem transaction: $ %s tx liquidstakeibc redeem 50000000qs/uatom`,
				version.AppName,
			),
		),
		Args: cobra.ExactArgs(1),
