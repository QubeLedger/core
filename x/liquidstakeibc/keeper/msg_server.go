...
func (k msgServer) RegisterHostChain(
	...
	if msg.Authority != k.authority && msg.Authority != k.GetParams(ctx).AdminAddress {
		return nil, sdkerrors.Wrapf(types.ErrInvalidSigner, "tx signer is not a module authority")
	}
...
func (k msgServer) LiquidStake(
	...
	if err != nil {
		return nil, sdkerrors.Wrapf(sdkerrors.ErrInvalidAddress, "error parsing delegator address: %s", err)
	}
...
func (k msgServer) LiquidStakeLSM(
	...
	for _, delegation := range msg.Delegations {
		...
		err = k.bankKeeper.MintCoins(ctx, types.ModuleName, sdktypes.NewCoins(mintToken))
		if err != nil {
			return nil, sdkerrors.Wrapf(types.ErrMintFailed, "failed to mint coins in module %s: %s", types.ModuleName, err)
		}
...
func (k msgServer) LiquidUnstake(
	...
	if !found {
		return nil, sdkerrors.Wrapf(types.ErrInvalidHostChain,
	...
	if msg.Amount.Denom != hc.MintDenom() {
		return nil, sdkerrors.Wrapf(types.ErrInvalidDenom,
			"expected %s, got %s",
			hc.MintDenom(),
			msg.Amount.Denom,
		)
...
func (k msgServer) validateLiquidStakeLSMDeposit(
	...
	if err := transfertypes.ValidateIBCDenom(delegation.Denom); err != nil {
		return nil, nil, nil, sdkerrors.Wrapf(types.ErrInvalidLSMDenom, "IBC denom %s doesn't belong to a LSM token", delegation.Denom)
	}
	...
	if err != nil {
		return nil, nil, nil, sdkerrors.Wrapf(err, "could not parse ibc hash from ibc hex %s", hexHash)
	}
	...
	if !found {
		return nil, nil, nil, sdkerrors.Wrapf(sdkerrors.ErrKeyNotFound, "host chain with channel id %s not registered", channelID)
	}
	...
	if validator.Status != stakingtypes.BondStatusBonded {
		return nil, nil, nil, sdkerrors.Wrapf(types.ErrLSMValidatorInvalidState, "validator %s is not in the bonded state, it is in %s", operatorAddress, validator.Status)
	}
	...
	if delegatorBalance.LT(delegation.Amount) {
		return nil, nil, nil, sdkerrors.Wrapf(sdkerrors.ErrInsufficientFunds, "not enough tokenized delegation funds")
	}
