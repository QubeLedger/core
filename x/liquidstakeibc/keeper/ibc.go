...
func (k *Keeper) OnChanOpenInit(
	...
	err := k.scopedKeeper.ClaimCapability(ctx, channelCap, host.ChannelCapabilityPath(portID, channelID))
	if err != nil {
		return "", err
	}
	return version, nil
...
func (k *Keeper) OnChanOpenAck(
	...
	_, portOwner, found := strings.Cut(portID, icatypes.PortPrefix) // #nosec
	if !found {
...
func (k *Keeper) OnAcknowledgementPacket(
	...
	if err := icatypes.ModuleCdc.UnmarshalJSON(packet.GetData(), &icaPacket); err != nil {
		return sdkerrors.Wrapf(sdkerrors.ErrUnknownRequest, "cannot unmarshal ICS-27 packet data: %v", err)
	}
...
func (k *Keeper) handleUnsuccessfulAck(
	...
	if err != nil {
		return sdkerrors.Wrapf(sdkerrors.ErrUnknownRequest, "cannot deserialize ica packet data: %v", err)
	}
...
func (k *Keeper) handleSuccessfulAck(
	...
	if err := k.cdc.Unmarshal(ack.GetResult(), txMsgData); err != nil {
		return sdkerrors.Wrapf(sdkerrors.ErrUnknownRequest, "cannot unmarshal ics-27 tx ack data: %v", err)
	}
	...
	for i, msg := range messages {
		...
		case sdk.MsgTypeURL(&stakingtypes.MsgUndelegate{}):
			...
			if len(txMsgData.Data) == 0 {
				data = txMsgData.GetData()[i].GetData()
			} else {
