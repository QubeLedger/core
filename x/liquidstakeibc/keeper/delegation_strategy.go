...
func (k *Keeper) GenerateDelegateMessages(hc *types.HostChain, depositAmount sdk.Int) ([]sdk.Msg, error) {
	...
	if len(delegableValidators) == 0 {
		return nil, sdkerrors.Wrap(types.ErrInvalidMessages, "no delegable validators")
	}
...
func (k *Keeper) GenerateUndelegateMessages(hc *types.HostChain, unbondAmount sdk.Int) ([]sdk.Msg, error) {
...
func (k *Keeper) generateMessages(
	...
	if len(messages) == 0 {
		err := sdkerrors.Wrap(types.ErrInvalidMessages, "no messages to delegate")
		if undelegating {
			err = sdkerrors.Wrap(types.ErrInvalidMessages, "no messages to undelegate")
		}
		return nil, err
	}
