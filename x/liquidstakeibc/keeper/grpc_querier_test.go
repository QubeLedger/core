...
const (
	...
	TestAddress string = "qube1gztc3y3k52hjds5nqvl7h9jvfnc33spzc4dnje"
)
...
func (suite *IntegrationTestSuite) TestQueryParams() {
	...
	tc := []struct {
	...
	}{
		{
			...
			resp: &types.QueryParamsResponse{
				Params: types.Params{
					AdminAddress: "qube1gztc3y3k52hjds5nqvl7h9jvfnc33spzc4dnje",
					FeeAddress:   "qube1gztc3y3k52hjds5nqvl7h9jvfnc33spzc4dnje",
				},
			},
		},
		{
	...
	for _, t := range tc {
		suite.Run(t.name, func() {
			resp, err := suite.app.LiquidStakeIBCKeeper.Params(sdktypes.WrapSDKContext(suite.ctx), t.req)
...
func (suite *IntegrationTestSuite) TestQueryHostChain() {
	...
	for _, t := range tc {
		suite.Run(t.name, func() {
			resp, err := suite.app.LiquidStakeIBCKeeper.HostChain(sdktypes.WrapSDKContext(suite.ctx), t.req)
...
func (suite *IntegrationTestSuite) TestQueryHostChains() {
	...
	for _, t := range tc {
		suite.Run(t.name, func() {
			resp, err := suite.app.LiquidStakeIBCKeeper.HostChains(sdktypes.WrapSDKContext(suite.ctx), t.req)
...
func (suite *IntegrationTestSuite) TestQueryDeposits() {
	...
	for _, t := range tc {
		suite.Run(t.name, func() {
			resp, err := suite.app.LiquidStakeIBCKeeper.Deposits(sdktypes.WrapSDKContext(suite.ctx), t.req)
...
func (suite *IntegrationTestSuite) TestQueryLSMDeposits() {
	...
	for _, t := range tc {
		suite.Run(t.name, func() {
			resp, err := suite.app.LiquidStakeIBCKeeper.LSMDeposits(sdktypes.WrapSDKContext(suite.ctx), t.req)
...
func (suite *IntegrationTestSuite) TestQueryUnbondings() {
	...
	for _, t := range tc {
		suite.Run(t.name, func() {
			resp, err := suite.app.LiquidStakeIBCKeeper.Unbondings(sdktypes.WrapSDKContext(suite.ctx), t.req)
...
func (suite *IntegrationTestSuite) TestQueryUserUnbondings() {
	...
	for _, t := range tc {
		suite.Run(t.name, func() {
			resp, err := suite.app.LiquidStakeIBCKeeper.UserUnbondings(sdktypes.WrapSDKContext(suite.ctx), t.req)
...
func (suite *IntegrationTestSuite) TestQueryHostChainUserUnbondings() {
	...
	suite.T().Run("ByOffset", func(t *testing.T) {
		for i := 0; i < PaginatedTestSize; i += PaginatedStep {
			resp, err := suite.app.LiquidStakeIBCKeeper.HostChainUserUnbondings(
				sdktypes.WrapSDKContext(suite.ctx),
	...
	suite.T().Run("Invalid Request", func(t *testing.T) {
		_, err := suite.app.LiquidStakeIBCKeeper.HostChainUserUnbondings(sdktypes.WrapSDKContext(suite.ctx), nil)
...
func (suite *IntegrationTestSuite) TestQueryValidatorUnbondings() {
	...
	for _, t := range tc {
		suite.Run(t.name, func() {
			resp, err := suite.app.LiquidStakeIBCKeeper.ValidatorUnbondings(sdktypes.WrapSDKContext(suite.ctx), t.req)
...
func (suite *IntegrationTestSuite) TestQueryUnbonding() {
	...
	for _, t := range tc {
		suite.Run(t.name, func() {
			resp, err := suite.app.LiquidStakeIBCKeeper.Unbonding(sdktypes.WrapSDKContext(suite.ctx), t.req)
...
func (suite *IntegrationTestSuite) TestQueryDepositAccountBalance() {
	...
	for _, t := range tc {
		suite.Run(t.name, func() {
			resp, err := suite.app.LiquidStakeIBCKeeper.DepositAccountBalance(sdktypes.WrapSDKContext(suite.ctx), t.req)
...
func (suite *IntegrationTestSuite) TestQueryExchangeRate() {
	...
	for _, t := range tc {
		suite.Run(t.name, func() {
			resp, err := suite.app.LiquidStakeIBCKeeper.ExchangeRate(sdktypes.WrapSDKContext(suite.ctx), t.req)
...
func (suite *IntegrationTestSuite) TestQueryRedelegations() {
	...
	for _, t := range tc {
		suite.Run(t.name, func() {
			resp, err := suite.app.LiquidStakeIBCKeeper.Redelegations(sdktypes.WrapSDKContext(suite.ctx), t.req)
...
func (suite *IntegrationTestSuite) TestQueryRedelegationTx() {
	...
	for _, t := range tc {
		suite.Run(t.name, func() {
			resp, err := suite.app.LiquidStakeIBCKeeper.RedelegationTx(sdktypes.WrapSDKContext(suite.ctx), t.req)
