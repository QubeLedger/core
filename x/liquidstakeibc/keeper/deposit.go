...
func (k *Keeper) AdjustDepositsForRedemption(
	...
	if depositsAmount.LT(redeemAmount.Amount) {
		return sdkerrors.Wrapf(liquidstakeibctypes.ErrInsufficientDeposits, "deposits are lesser than amount to be redeemed, deposits present %s, required %s", depositsAmount.String(), redeemAmount.Amount.String())
	}
...
func (k *Keeper) GetRedeemableDepositsForHostChain(
	ctx sdk.Context,
	hc *liquidstakeibctypes.HostChain,
) ([]*liquidstakeibctypes.Deposit, sdk.Int) {
	store := prefix.NewStore(ctx.KVStore(k.storeKey), liquidstakeibctypes.DepositKey)
	iterator := sdk.KVStorePrefixIterator(store, nil)
	defer iterator.Close()
